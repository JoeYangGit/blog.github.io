  ...
  <div>
    <div class="feeling">
      <div class="feeling-left">
        <div class="feeling-left-img"></div>
        <div class="feeling-left-title">全部随笔</div>
      </div>
      <div class="feeling-list">
        <div class="feeling-list-left">
          <div class="feeling-list-left-circle" :style="leftColor()" @click="prevPage()"></div>
        </div>
        <div class="feeling-list-content">
          <div class="feeling-list-content-box">
            <div
              class="feeling-list-content-item"
              v-for="(item, index) in listData1"
              :key="item.id"
              @click="showDetailBox(item)"
            >
              <div class="feeling-list-content-item-summary">
                <div
                  class="feeling-list-content-item-summary-title"
                >{{currentIndex - 5 + index + '. ' + item.text}}</div>
                <div class="feeling-list-content-item-summary-time">{{item.time}}</div>
              </div>
            </div>
            <template v-if="listData1.length < 3">
              <div class="feeling-list-content-item-empty" v-for="item in emptyData" :key="item"></div>
            </template>
          </div>
          <div class="feeling-list-content-box">
            <div
        ...
        <div class="feeling-list-right">
          <div class="feeling-list-right-circle" :style="rightColor()" @click="nextPage()"></div>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted () {
    this.listData = [...this.listData, ...latestFeeling].reverse()
